...
Schema can be constructed by creating `doctype` followed by its `property` (if any), each of which can have its own `label` and `description`. For example in WOQLjs:
